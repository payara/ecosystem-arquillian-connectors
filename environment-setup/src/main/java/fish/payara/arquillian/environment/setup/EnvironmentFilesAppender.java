...
import static fish.payara.arquillian.environment.setup.EnviromentRemoteExtension.ENV_PROP_FILE;
import static fish.payara.arquillian.environment.setup.EnviromentRemoteExtension.SYS_PROP_FILE;
import java.net.URL;
...
public class EnvironmentFilesAppender implements ApplicationArchiveProcessor {
    ...
    public void process(Archive<?> applicationArchive, TestClass testClass) {
        if (applicationArchive instanceof ResourceContainer) {
            ...
            URL envVarURL = Thread.currentThread().getContextClassLoader().getResource(ENV_PROP_FILE);
            if (envVarURL != null) {
                container.addAsResource(envVarURL, ENV_PROP_FILE);
            }
            URL sysPropsURL = Thread.currentThread().getContextClassLoader().getResource(SYS_PROP_FILE);
            if (sysPropsURL != null) {
